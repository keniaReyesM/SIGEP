<template>
  <div>
    <div class="container">
      ...
      <v-row center align="center" justify="center">
        <v-col md="6" cols="12">
          <validation-provider v-slot="{ errors, valid }" name="Razón social" rules="required|max:100">
            <v-text-field :counter="100" maxlength="100" v-model.trim="model.razonSocial" label="Razón social*" outlined
      ...
      <div class="text-center">
        <v-tooltip top>
          <template v-slot:activator="{ on, attrs }">
            <v-btn icon>
              <v-icon v-bind="attrs" v-on="on" @click="agregarRepresentante()" color="primary" left dense>
                mdi-plus-circle-outline
              </v-icon>
            </v-btn>
          </template>
          <small>Agregar representante</small>
        </v-tooltip>
      </div>
      <v-row v-for="(representante, indice) in model.representantes" :key="`${indice}representante`" class="mb-5 mt-2">
        <v-row>
          <v-col md="1" cols="12">
            #{{ indice + 1 }}
          </v-col>
          <v-col md="4" cols="12">
            <validation-provider v-slot="{ errors, valid }" name="Nombre" rules="required|max:100|'alpha'">
              <v-text-field :counter="100" maxlength="100" v-model.trim="representante.nombre" label="Nombre*" outlined
                clearable required autocomplete="off" :error-messages="errors" dense clear-icon="mdi-eraser">
                <v-icon slot="append" :color="valid ? 'success' : ''">
                  {{ valid? " mdi-check": "" }}
                </v-icon>
          ...
          <v-col md="4" cols="12">
            <validation-provider v-slot="{ errors, valid }" name="Primer apellido" rules="required|max:100|'alpha'">
              <v-text-field :counter="100" maxlength="100" v-model.trim="representante.primerApellido"
                label="Primer apellido*" outlined clearable required autocomplete="off" :error-messages="errors" dense
          ...
          <v-col md="2" cols="12">
            <div class="text-center" v-if="indice != 0">
              <v-tooltip top>
                <template v-slot:activator="{ on, attrs }">
                  <v-btn icon @click="eliminarRepresentante(indice)">
          ...
          <v-col md="4" offset="1" cols="12">
            <validation-provider v-slot="{ errors, valid }" name="Segundo apellido" rules="max:100|'alpha'">
              <v-text-field :counter="100" maxlength="100" v-model.trim="representante.segundoApellido"
                ...
                <v-icon slot="append" :color="valid ? 'success' : ''">
                  {{ valid && representante.segundoApellido ? " mdi-check" : "" }}
                </v-icon>
              </v-text-field>
            </validation-provider>
          </v-col>
          <v-col md="4" cols="12">
            <validation-provider v-slot="{ errors, valid }" name="Fecha de nacimiento" rules="required">
              <v-text-field type="date" v-model.trim="representante.fechaNacimiento" :max="new Date()"
                label="Fecha de nacimiento" outlined clearable required autocomplete="off" :error-messages="errors"
                dense clear-icon="mdi-eraser">
                <v-icon slot="append" :color="valid ? 'success' : ''">
                  {{ valid? " mdi-check": "" }}
                </v-icon>
      ...
      <v-row v-for="(correo, indice) in model.correos" :key="`${indice}correo`">
        ...
        <v-col offset="3" md="2" cols="12">
          <validation-provider v-slot="{ errors }" name="Principal" rules="required">
            <v-switch v-model="correo.principal" inset label="Principal" required autocomplete="off"
              :error-messages="errors" dense />
          </validation-provider>
        </v-col>
        <v-col md="2" cols="12">
