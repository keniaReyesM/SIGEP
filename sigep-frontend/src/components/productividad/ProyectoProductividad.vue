        ...
        <div class="contxxxent">
            ...
            <div class="mb-3 ">
                ...
                <div class="card-header">
                    <v-row align="center" justify="center" class="ml-2 mr-2">
                        ...
                        <v-col cols="12" sm="12" md="8" align="center" justify="center">
                            ...
                            <v-autocomplete dense outlined clearable required auto-select-first autocomplete="off"
                                :items="proyectos" item-text="nombre" item-value="idProyecto" v-model.trim="idProyecto"
                                label="Seleccione proyecto*" @change="listarPaginado" clear-icon="mdi-eraser"
                                no-data-text="Sin resultados disponibles." />
                        </v-col>
                        <!-- <v-col cols="12" sm="12" md="4" align="center" justify="center">

                            <v-btn @click="descargarReporte()" :disabled="enviando || lista.length == 0" outlined rounded
                    ...
                    <v-row v-if="proyecto" class="ml-2 mr-2 mt-2 mb-2" dense>
                        <v-col cols="12" sm="12" md="4"  align="center" justify="center">
                            <v-icon class="mr-1">
                                mdi-human-male-female
                            </v-icon>
                            <span class="subheading mr-2">
                                Empleados: {{ proyecto.empleados.length }}
                            </span>
                        ...
                        <v-col cols="12" sm="12" md="4"  align="center" justify="center">
                            ...
                            <span class="subheading">
                                Clientes: {{ proyecto.clientes.join(", ") }}
                            </span>
                        </v-col>
                    </v-row>
    ...
    data() {
        return {
            ...
            headers: [
                ...
                { text: "Empleado", value: "nombreCompleto", align: "center", sortable: false, },
                { text: "Horas trabajadas", value: "horasTrabajadas", align: "center", sortable: false, width: "40%", }
            ]
        }
    },
    watch: {
    ...
    methods: {
        ...
        listarPaginado() {
            ...
            if (!this.enviando) {
                ...
                if (this.$utileria.isNull(this.idProyecto)) {
                    this.lista = [];
                    this.total = 0;
                    ...
                    this.obtenerInformacionProyecto();
                    ...
                    ProyectoService.listarPaginadoProductividadProyecto(parametros).then((res) => {
        ...
        listarProyectos() {
            ProyectoService.listarTodos().then((response) => {
                this.proyectos = response.data.map((obj) => {
                    return { idProyecto: obj.idProyecto, nombre: `${obj.clave} - ${obj.nombre}` }
                })
            });
        },
        descargarReporte() {
            if (!this.enviando) {
                ...
                this.enviando = true;
                let parametros = { idProyecto: this.idProyecto };

                ProyectoService.generarReporteProductividadProyecto(parametros).then((respuesta) => {
                    this.$utileria.descargarExcel(respuesta.data, `${this.proyecto.clave} ${this.proyecto.nombre}.xlsx`);
                }).catch((e) => {
                    this.$utileria.errorhttp(e);
                }).finally(() => {
                    this.enviando = false;
        ...
        obtenerInformacionProyecto() {
            ...
            ProyectoService.obtenerInformacionGeneral(this.idProyecto).then((response) => {
                ...
                this.totalHoras = this.proyecto.horasTrabajadas;
